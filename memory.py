...
class Array:
	...
	def __setitem__(self, index, value):
		if index is Ellipsis or (hasattr(index, 'start') and hasattr(index, 'stop') and hasattr(index, 'step')):
			...
			for index, v in zip(sm_range(start, stop, step), value):
				self[index] = v
...
class Table:
	...
	def __init__(self, items, ksize=None, sizes=None, types=None, start=None, stop=None, Array=None):
		try:
			...
			if ksize is None:
				...
				key = next(iter(keys))
				ksize = [max(_k[_n] for _k in keys) + 1 for _n in sm_range(len(key))]
			...
			if isinstance(items, self.StorageType):
				...
				indices = sm_range(ksize[0])
				for s in ksize[1:]:
					indices = product(indices, sm_range(s))
	...
	def keys(self):
		indices = sm_range(self.__ksize[0])
...
if __debug__ and __name__ == '__main__':
	...
	if False:
		...
		class NpArray(Array):
			...
			Storage = lambda x: array(list(x), dtype=uint8)
			
			def __eq__(self, other):
				try:
					return (self._Array__storage == other._Array__storage).all()
				except AttributeError:
					return NotImplemented
		...
		class NpTable(Table):
			...
			def __eq__(self, other):
				try:
					return (self._Table__storage == other._Table__storage).all()
				except AttributeError:
					return NotImplemented
	...
	'''
	if True:
		from optimized import Collection, Value
		
		class OptArray(Array):
			StorageType = Collection
			Storage = Collection
			cast = lambda x: x
		
		class OptTable(Table):
			StorageType = Collection
			Storage = Collection
			cast = lambda x: x
	...
	F.field_size = 256
	F.Field = F
	F.random = lambda randbelow: F(randbelow(F.field_size))
	...
	for F in [Galois('F3', 3, [1, 0, 2, 1]), Galois('Rijndael', 2, [1, 0, 0, 0, 1, 1, 0, 1, 1])]:
		...
		ann[...] = [[[F(m * n + k) for n in sm_range(4)] for m in sm_range(3)] for k in sm_range(2)]
		assert ann[0][2][3] == F(6)
		assert ann[0][1][2] == F(2)
		assert ann[1][2][3] == F(7)
		assert ann[1][1][2] == F(3)
		
		l1 = Linear(Array([F(_n + 1) for _n in sm_range(F.field_power)], [None], [F]))
		l2 = Linear.random(Array, F, randrange)
		print(l1, l2, l1 + l2)
		
		q1 = Quadratic(Array([Linear(Array([F((_n + _m * F.field_power + 1)  % F.field_size) for _n in sm_range(F.field_power)], [None], [F])) for _m in sm_range(F.field_power)], [F.field_power, None], [Linear, F]))
		...
		with PyCallGraph(output=GraphvizOutput(output_file=f'linear_linear_{F.__name__}.png')):
			print()
			a = Automaton.random_linear_linear(4, 4, 8, Table, Array, Vector, LinearCircuit, Linear, F, randrange)
			s = a.init_state[:]
			print(s)
			for n, x in enumerate(a(random_stream(2, 4, Array, F, randrange), s)):
				print(n, x)
			print(s)
		
		with PyCallGraph(output=GraphvizOutput(output_file=f'linear_quadratic_{F.__name__}.png')):
			print()
			b = Automaton.random_linear_quadratic(4, 4, 8, Table, Array, Vector, QuadraticCircuit, LinearCircuit, Quadratic, Linear, F, randrange)
			s = b.init_state[:]
			print(s)
			for n, x in enumerate(b(random_stream(2, 4, Array, F, randrange), s)):
				print(n, x)
			print(s)
		
		with PyCallGraph(output=GraphvizOutput(output_file=f'quadratic_linear_{F.__name__}.png')):
			print()
			c = Automaton.random_quadratic_linear(4, 4, 8, Table, Array, Vector, QuadraticCircuit, LinearCircuit, Quadratic, Linear, F, randrange)
			s = c.init_state[:]
			print(s)
			for n, x in enumerate(c(random_stream(2, 4, Array, F, randrange), s)):
				print(n, x)
			print(s)
		
		with PyCallGraph(output=GraphvizOutput(output_file=f'quadratic_quadratic_{F.__name__}.png')):
			print()
			d = Automaton.random_quadratic_quadratic(4, 4, 8, Table, Array, Vector, QuadraticCircuit, Quadratic, Linear, F, randrange)
			s = d.init_state[:]
			print(s)
			for n, x in enumerate(d(random_stream(2, 4, Array, F, randrange), s)):
				print(n, x)
			print(s)
